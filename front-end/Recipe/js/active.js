(function ($) {
    ...
    if ($.fn.scrollUp) {
        $.scrollUp({
            scrollSpeed: 1500,
            scrollText: '<i class="pe-7s-angle-up" aria-hidden="false"></i>'
        });
    }
   ...
   /* // Sticky Active JS
    ...
    function hasScrolled() {
        ...
        if (st > lastScrollTop && st > navbarHeight){
            // Scroll Down
            $('navbar-nav').removeClass('nav-down').addClass('nav-up');
        } else {
            // Scroll Up
            if(st + $(window).height() < $(document).height()) {
                $('navbar-nav').removeClass('nav-up').addClass('nav-down');
            }
        }
